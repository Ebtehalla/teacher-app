...
class UserService {
  ...
  /// return empty string if current user don't have profile picture
  ...
  Future<String?> getTeacherId() async {
    try {
      var docRef = await FirebaseCollection()
          ...
          .get();
      if (docRef.exists) {
        if (docRef.data()?.teacherId != null) {
          return docRef.data()?.teacherId;
        } else {
          return null;
        }
      } else {
        return Future.error(
            'cannot get teacher data, teacher data with is null');
      }
    } catch (e) {
      return Future.error(e.toString());
    }
