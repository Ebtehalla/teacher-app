...
import 'package:halo_teacher/app/models/repeat_timeslot_model.dart';
...
class AddTimeslotController extends GetxController {
  ...
  TeacherCalendarController teacherCalendarController = Get.find();
  ...
  void addTimeslot() async {
    try {
      ...
      if (validationSuccess) {
        ...
        teacherCalendarController.updateEventsCalendar();
        Get.back();
      }
    } catch (e) {
  ...
  Future<String> addOneTimeSlot({bool isParent = false}) async {
    String timeSlotId = await TimeSlotService().saveTeacherTimeslot(
    ...
    if (price! < currentTeacher!.basePrice!) {
      await TeacherService().updateTeacherBasePrice(price!);
    }
    return timeSlotId;
  }
